...
@if(abs($value) > 0.01)
    <span class="d-flex align-items-center gap-1 extra-small fw-bold {{ $value >= 0 ? 'text-success' : 'text-danger' }}">
        <svg class="{{ $value < 0 ? 'rotate-180' : '' }}" width="10" height="10" viewBox="0 0 12 12" fill="currentColor"
